...
interface ClientBookingRatingFormProps extends DrawerButtonContentProps {
  booking: FindBookingsResponse;
}

export function ClientBookingRatingForm({ booking, onCancel }: ClientBookingRatingFormProps) {
    ...
    mutation: {
      onSuccess: async () => {
        ...
        await Promise.all([
          ...
          queryClient.invalidateQueries({ queryKey: getFindRatingsQueryKey() }),
        ]);
  ...
  if (isSuccess) {
    return (
      <Stack gap="md" align="center" py="md">
        ...
        <Button onClick={onCancel} fullWidth mt="md">
          {t('actions.done')}
        </Button>
      </Stack>
    );
