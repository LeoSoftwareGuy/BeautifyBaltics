...
import {
  Card, Drawer, Stack, Text, ThemeIcon,
} from '@mantine/core';
import { IconPlus } from '@tabler/icons-react';

import useRoutedModal from '@/hooks/use-routed-modal';

import { AddMasterServicesForm } from './master-services-form';

type AddServiceCardProps = {
  masterId: string;
};

export function AddServiceCard({ masterId }: AddServiceCardProps) {
  const [isHovered, setIsHovered] = useState(false);
  const { t } = useTranslation();
  const { onOpen, opened, onClose } = useRoutedModal('add-service');
    ...
    <>
      ...
      >
        <Suspense fallback={null}>
          {opened ? <AddMasterServicesForm masterId={masterId} onCancel={onClose} /> : null}
        </Suspense>
      </Drawer>
    </>
  );
